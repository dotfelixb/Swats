...
const OpenHourItem: FC<IOpenHourItem> = ({ data, hoursList, setHoursList }) => {
  ...
  const onChange = (e: any) => {
    ...
    const newItem: IOpenHour = {
      ...newData, enabled: e.target.checked
    };
  ...
  const onFromChange = (e: any) => {
    ...
    // create a new hour
    const newItem: IOpenHour = {
      ...newData, fromTime: dayjs(e).format()
    };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
      ...
      <Form.Item name={`${data.name}from`} valuePropName="checked">
        <DatePicker placeholder="From" onChange={onFromChange} picker="time" disabled={dateDisabled} />
      </Form.Item>
      <Form.Item name={`${data.name}to`} valuePropName="checked">
        <DatePicker placeholder="To" onChange={onToChange} picker="time" disabled={dateDisabled} />
      </Form.Item>
    </div>
  );
};
