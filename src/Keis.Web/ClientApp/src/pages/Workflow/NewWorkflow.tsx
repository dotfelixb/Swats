import {ClockCircleOutlined, CommentOutlined, FireOutlined, FormOutlined,} from "@ant-design/icons";
...
import {IListResult, ISingleResult, IWorkflowAction, IWorkflowCriteria, IWorkflowEvent,} from "../../interfaces";
...
const NewWorkflow: FC<INewWorkflow> = () => {
    ...
    const [workflowCriteriaList, setWorkflowCriteriaList] = useState<IWorkflowCriteria[]>([]);
    const [workflowActionList, setWorkflowActionList] = useState<IWorkflowAction[]>([]);
    ...
    const onFinish = async (values: IFormData) => {
        ...
        for (let index = 0; index < actionList.length; index++) {
            body.append(`actions[${index}].name`, actionList[index].name?.toString() ?? "");
            body.append(`actions[${index}].actionFrom`, actionList[index].actionFrom?.toString() ?? "");
            body.append(`actions[${index}].actionTo`, actionList[index].actionTo ?? "");
        }

        for (let index = 0; index < criteriaList.length; index++) {
            body.append(`criteria[${index}].name`, criteriaList[index].name?.toString() ?? "");
            body.append(`criteria[${index}].criteria`, criteriaList[index].criteria?.toString() ?? "");
            body.append(`criteria[${index}].condition`, criteriaList[index].condition?.toString() ?? "");
                    ...
                    <Form form={form} layout="vertical" onFinish={onFinish}>
                        ...
                        <Timeline>
                            ...
                            >
                                ...
                                {criteriaList?.sort((l, r) => l.key - r.key).map((c) => (
                                    <WorkflowCriteria
                                        key={c.key}
                                        id={c.key}
                            ...
                            >
                                ...
                                {actionList?.sort((l, r) => l.key - r.key).map((c) => (
                                    <WorkflowAction
                                        key={c.key}
                                        id={c.key}
